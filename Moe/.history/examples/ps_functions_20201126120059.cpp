...
int main()
{
    //this functions are equivalent as the stated inputs are just the same
    //as the given inputs
    ...
    moether.setFitnessFunction( [](auto moe) -> double
    {
        float   x = moe.genotype[0],
                y = moe.genotype[1]; // don´t know if we can add more variables
                //z = moe.genotype[2];


        //float   result = f(x, y);
        //float result = easom(x,y);
        //float result = (3*x + 2*y);
        float result = 3*x + 2*y;
        return -result;
    });
    ...
    auto start = std::chrono::high_resolution_clock::now();
        ...
        moether.run( 1000 ); //the run function can be found in include/moe/base/algorithms/particleSwarmOptimization
