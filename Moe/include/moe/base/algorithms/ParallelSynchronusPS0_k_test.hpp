...
class ParticleSwarmPS_k_t : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPS_k_t( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPS_k_t( const PSParameters<GenotypeType>& _parameters );
    ...
    private:
        ...
        std::chrono::duration<GenotypeType> diff;
...
template <typename GenotypeType>
ParticleSwarmPS_k_t<GenotypeType>::ParticleSwarmPS_k_t( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPS_k_t<GenotypeType>::ParticleSwarmPS_k_t( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPS_k_t<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{
    
}

template <typename GenotypeType>
void ParticleSwarmPS_k_t<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
std::vector<GenotypeType> ParticleSwarmPS_k_t<GenotypeType>::getRandomGenotype()
{
...
template <typename GenotypeType>
void ParticleSwarmPS_k_t<GenotypeType>::init( unsigned int _iterations, int _particle_per_rank)
{
...
template <typename GenotypeType>
void ParticleSwarmPS_k_t<GenotypeType>::run( unsigned int _iterations)
{ 
    ...
    for( unsigned int i = 0; i < m_iterations; ++i )
    {
        ...
        if(rank == 0 && i % 4 == 0) 
        {
            ...
            opt_of_iter = global_best_recv_g[2];
            //std::cout<<"first recieved in gather: " << opt_of_iter << "\n";
            for(unsigned j = 0; j < size; ++j) {
                if(global_best_recv_g[j*3+2] < opt_of_iter) {
                    opt_of_iter = global_best_recv_g[j*3+2];
                    k = j*3+2;
                    //std::cout << "k: " << k << "\n";
                    //std::cout << "opt_of_iter: " << opt_of_iter << "         m_best_genotypes[j].fitness: " << m_best_genotypes[j].fitness << "           Algorithm<GenotypeType>::m_bestMoe.fitness: " << Algorithm<GenotypeType>::m_bestMoe.fitness << std::endl;
                }
            }
            //std::cout << "m_bestMoe: " << Algorithm<GenotypeType>::m_bestMoe.fitness << ", opt_of_iter: " << opt_of_iter << std::endl;
            //std::cout << "size of recv: " << global_best_recv_g[k] << std::endl;
            if( opt_of_iter < Algorithm<GenotypeType>::m_bestMoe.fitness )
                { 
                    ...
                    //std::cout << "opt_of_iter: " << opt_of_iter << "         m_best_genotypes[j].fitness: " << m_best_genotypes[k].fitness << "           Algorithm<GenotypeType>::m_bestMoe.fitness: " << Algorithm<GenotypeType>::m_bestMoe.fitness << std::endl;
                    //Algorithm<GenotypeType>::m_bestMoe.fitness = opt_of_iter; //changed from m_bestMoe to m_bestMoe.fitness
    ...
    //if(rank == 0) std::cout << "Time elapsed in master loop: " << diff.count() << std::endl;
