...
class ParticleSwarmPS_k_RB : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPS_k_RB( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPS_k_RB( const PSParameters<GenotypeType>& _parameters );
...
template <typename GenotypeType>
ParticleSwarmPS_k_RB<GenotypeType>::ParticleSwarmPS_k_RB( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPS_k_RB<GenotypeType>::ParticleSwarmPS_k_RB( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPS_k_RB<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{
    
}

template <typename GenotypeType>
void ParticleSwarmPS_k_RB<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
std::vector<GenotypeType> ParticleSwarmPS_k_RB<GenotypeType>::getRandomGenotype()
{
...
template <typename GenotypeType>
void ParticleSwarmPS_k_RB<GenotypeType>::init( unsigned int _iterations, int _particle_per_rank)
{
...
template <typename GenotypeType>
void ParticleSwarmPS_k_RB<GenotypeType>::run( unsigned int _iterations)
{ 
    ...
    for( unsigned int i = 0; i < m_iterations; ++i )
    {
        ...
        MPI_Reduce(&global_best_send_g, &global_best_recv_g, 1, MinMoeType, min_moe_op, 0, MPI_COMM_WORLD);

        //std::cout << "Checkpoint -1/2: pre Bcast!" << std::endl;

        //MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(&global_best_recv_g, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
