...
class ParticleSwarmPA_prec_vel : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPA_prec_vel( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPA_prec_vel( const PSParameters<GenotypeType>& _parameters );
...
template <typename GenotypeType>
ParticleSwarmPA_prec_vel<GenotypeType>::ParticleSwarmPA_prec_vel( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPA_prec_vel<GenotypeType>::ParticleSwarmPA_prec_vel( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPA_prec_vel<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{
    
}

template <typename GenotypeType>
void ParticleSwarmPA_prec_vel<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
void ParticleSwarmPA_prec_vel<GenotypeType>::run( unsigned int _iterations)
{
    int rank, numproces;
...
//                std::cout<<"Checkpoint 2: while loop step: "<< step <<std::endl;
                ...
                MPI_Send(our_send_particle, 7, MPI_DOUBLE, status.MPI_SOURCE, tag, MPI_COMM_WORLD);
        ...
        for(int id = 1; id < numproces; ++id){
            ...
            MPI_Send(our_recv_particle, 7, MPI_DOUBLE, status.MPI_SOURCE, 0, MPI_COMM_WORLD);
        }
