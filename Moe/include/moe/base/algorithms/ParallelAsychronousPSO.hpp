...
void ParticleSwarmPA<GenotypeType>::run( unsigned int _iterations, int& argc, char * argv[])
{
    ...
    double particles_to_scatter[(numproces-1)*4];
    double particle_to_recieve[4];
    if(rank == 0){
        ...
        for(int j = 1; j<(numproces); ++j){
            particles_to_scatter[j*4] = m_population[j-1].genotype[0];
            particles_to_scatter[j*4 + 1] = m_population[j-1].genotype[1];
            particles_to_scatter[j*4 + 2] = m_population[j-1].fitness;
            particles_to_scatter[j*4 + 3] = j-1;
        }
...
//                std::cout<<"Checkpoint 2: while loop step: "<< step <<std::endl;
                ...
                MPI_Send(our_current_particle, 3, MPI_DOUBLE, status.MPI_SOURCE, tag, MPI_COMM_WORLD);
    ...
    if(rank != 0){ //slave process
        ...
        int curr_particle_number = (int)(double)particle_to_recieve[3];
        ...
        MPI_Send(current_position, 3, MPI_DOUBLE, 0, curr_particle_number, MPI_COMM_WORLD);
        ...
        while(1){
            ...
            MPI_Send(current_position, 3, MPI_DOUBLE, 0, particle_number, MPI_COMM_WORLD);
