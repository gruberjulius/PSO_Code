...
class ParticleSwarmPA_send_move_nnblc : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPA_send_move_nnblc( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPA_send_move_nnblc( const PSParameters<GenotypeType>& _parameters );
...
template <typename GenotypeType>
ParticleSwarmPA_send_move_nnblc<GenotypeType>::ParticleSwarmPA_send_move_nnblc( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPA_send_move_nnblc<GenotypeType>::ParticleSwarmPA_send_move_nnblc( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPA_send_move_nnblc<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{

}

template <typename GenotypeType>
void ParticleSwarmPA_send_move_nnblc<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
void ParticleSwarmPA_send_move_nnblc<GenotypeType>::run( unsigned int _iterations){std::cout<<"The wrong \" run \" function has been called in ParticleSwarmPA_send_move_nnblc.cpp"<<std::endl;}
template <typename GenotypeType>
void ParticleSwarmPA_send_move_nnblc<GenotypeType>::run( unsigned int _iterations, int& argc, char * argv[])
{
    int rank, numproces;
...
//                std::cout<<"Checkpoint 2: while loop step: "<< step <<std::endl;
                ...
                MPI_Isend(our_current_particle, 2, MPI_DOUBLE, status.MPI_SOURCE, tag, MPI_COMM_WORLD,&unused_request);
        ...
        for(int id = 1; id < numproces; ++id){
            ...
            MPI_Isend(our_current_particle, 2, MPI_DOUBLE, status.MPI_SOURCE, 0, MPI_COMM_WORLD,&unused_request);
        }
    ...
    if(rank != 0){ //slave process
        ...
        while(1){
            ...
            //std::cout << "Rank: " << rank << ", spent " << diff.count() << " time on eval\n";
