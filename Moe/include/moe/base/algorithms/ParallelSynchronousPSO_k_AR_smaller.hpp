...
class ParticleSwarmPS_k_aR_smaller : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPS_k_aR_smaller( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPS_k_aR_smaller( const PSParameters<GenotypeType>& _parameters );
...
template <typename GenotypeType>
ParticleSwarmPS_k_aR_smaller<GenotypeType>::ParticleSwarmPS_k_aR_smaller( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPS_k_aR_smaller<GenotypeType>::ParticleSwarmPS_k_aR_smaller( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPS_k_aR_smaller<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{

}

template <typename GenotypeType>
void ParticleSwarmPS_k_aR_smaller<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
std::vector<GenotypeType> ParticleSwarmPS_k_aR_smaller<GenotypeType>::getRandomGenotype()
{
...
template <typename GenotypeType>
void ParticleSwarmPS_k_aR_smaller<GenotypeType>::init( unsigned int _iterations, int _particle_per_rank)
{
...
//FLORIAN EDIT START
struct particlereduce_f{
        float genotype1;
        float genotype2;
        double fitness;
        };
//void min_fit_f(particlereduce_f*, particlereduce_f*, int*, MPI_Datatype*);
void min_fit_f(particlereduce_f* invec, particlereduce_f* inoutvec, int* len, MPI_Datatype *datatype){
    for(int i = 0; i < *len; ++i){
...
template <typename GenotypeType>
void ParticleSwarmPS_k_aR_smaller<GenotypeType>::run( unsigned int _iterations)
{
    ...
    MPI_Op min_moe_op;
    MPI_Op_create( (MPI_User_function *) min_fit_f,0,&min_moe_op);
    ...
    particlereduce_f global_best_send_g = {Algorithm<GenotypeType>::m_bestMoe.genotype[0], Algorithm<GenotypeType>::m_bestMoe.genotype[1], Algorithm<GenotypeType>::m_bestMoe.fitness};
    particlereduce_f global_best_recv_g[size];
