...
class ParticleSwarmPA_send_move_termcrit : public NumericAlgorithm<GenotypeType>
{
    public:
        ParticleSwarmPA_send_move_termcrit( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions = 1, std::vector<GenotypeType> _range = { std::numeric_limits<GenotypeType>::lowest() , std::numeric_limits<GenotypeType>::max() });
        ParticleSwarmPA_send_move_termcrit( const PSParameters<GenotypeType>& _parameters );
...
template <typename GenotypeType>
ParticleSwarmPA_send_move_termcrit<GenotypeType>::ParticleSwarmPA_send_move_termcrit( unsigned int _moesPerGen, float _weight, float _coef1, float _coef2, unsigned int _dimensions, std::vector<GenotypeType> _range )
...
template <typename GenotypeType>
ParticleSwarmPA_send_move_termcrit<GenotypeType>::ParticleSwarmPA_send_move_termcrit( const PSParameters<GenotypeType>& _parameters )
:ParticleSwarmPA_send_move_termcrit<GenotypeType>( _parameters.moesPerGen, _parameters.inertia, _parameters.coef1, _parameters.coef2, _parameters.dimensions, _parameters.range )
{

}

template <typename GenotypeType>
void ParticleSwarmPA_send_move_termcrit<GenotypeType>::init( unsigned int _iterations )
{
...
template <typename GenotypeType>
void ParticleSwarmPA_send_move_termcrit<GenotypeType>::run( unsigned int _iterations){std::cout<<"The wrong \" run \" function has been called in ParticleSwarmPA_send_move_termcrit.cpp"<<std::endl;}
template <typename GenotypeType>
void ParticleSwarmPA_send_move_termcrit<GenotypeType>::run( unsigned int _iterations, int& argc, char * argv[])
{
    int rank, numproces;
...
//FLORIAN EDIT START
        double err = 1e-7;
        std::list<int> particledeath;//, available_threads; //queue-s of particle indexes and available thread ranks
        //initialization of particle queue with numproces-1,...,#particles
        for(int i = 0; i < num_particles; ++i){ //numproces-1 because we already scattered
            particledeath.push_back(i);
        }
//FLORIAN EDIT END

        double modified_iterations = num_particles * m_iterations;
        while(curr_sum_iters < modified_iterations && !particledeath.empty() )//was step < m_iterations
        {
//                std::cout<<"Checkpoint 2: while loop step: "<< step <<std::endl;

                MPI_Status status;
                MPI_Recv(our_current_particle, 1, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);//need MPI_Datatype of Particle

                int current_idx = status.MPI_TAG; //particle index recieved in tag. Send particle idx + 1, to know when to terminate
//FLORIAN EDIT START
// if |new-old| <= err   we near completion
                if(std::abs(our_current_particle[0] - m_population[current_idx].fitness) < err){
                    std::list<int>::iterator part_it = particledeath.begin();
                    std::advance(part_it,current_idx);
                    particledeath.erase(part_it);
                }
//FLORIAN EDIT END
                m_population[current_idx].fitness = our_current_particle[0];
        ...
        for(int id = 1; id < numproces; ++id){
            MPI_Status status;
            MPI_Recv(our_current_particle, 1, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
            int particle_number = status.MPI_TAG;
            m_population[particle_number].fitness = our_current_particle[0];
