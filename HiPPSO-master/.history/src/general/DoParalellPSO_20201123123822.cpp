...
Statistics* DoParalellPso(Statistics* statistics, time_t& LAST_BACKUP, time_t& LAST_RUN_CHECK, int& argc, char * argv[]) {
    ...
    //////////////////////////////EDIT END
        ...
        while(step < configuration::g_max_iterations){//while iterations
            int current_idx = particlequeue.pop(); //get new particle idx
            ...
            int next_available = available_threads.pop(); //next available thread, from queue for available threads

            std::vector<double> vel = MpftToDouble(current_particle.GetPostition());
            double* position_mpi = vel.data();
            
            //transform to non blocking, buffers, #makethiscodegreatagain

            MPI_Send(position_mpi, 2, MPI_DOUBLE, next_available, 1, MPI_COMM_WORLD);//need MPI_Datatype of Particle*
            double cost_value; //make into array to have threads buffer to avoid deadlock
            MPI_Recv(cost_value, 1, MPI_DOUBLE, next_available, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
////////////////////////////////
    if(rank != 0){ //slave process
        ...
        MPI_Recv(current_particle, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);//need MPI_Datatype of Particle*
        std::vector<mpf_t*> currPos = current_particle->GetPosition();
        cost_value = FunctionEvaluationReduceOperation::Evaluate(currPos);
        MPI_Send(cost_value, 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
    }
    MPI_Finalize();
