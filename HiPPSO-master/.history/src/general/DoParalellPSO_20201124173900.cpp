...
Statistics* DoParalellPso(Statistics* statistics, time_t& LAST_BACKUP, time_t& LAST_RUN_CHECK, int& argc, char * argv[]) {
    ...
    //////////////////////////////EDIT END
        ...
        while(step < configuration::g_max_iterations){//while iterations
            ...
            MPI_DOUBLE* position_mpi = vel.data();
            ...
            MPI_DOUBLE cost_value; //make into array to have threads buffer to avoid deadlock
        //  MPI_Send(cost_value, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD); the stuff that is sended
   
            MPI_Recv(&cost_value, 1, MPI_DOUBLE, next_available, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //passed by reference as recommended
...
////////////////////////////////
    if(rank != 0){ //slave process
        ...
        //cost_value = FunctionEvaluationReduceOperation::Evaluate( & currPos);  //should be passed by reference
