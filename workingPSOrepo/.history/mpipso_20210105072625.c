...
double x;
double y;
double result(double x, double y); //, how did we do this?
...
int main(int argc, char *argv[]) {
    ...
    switch(s_case) { // switch case to set correct range of function
        case 0: 
            double range[2] = {-600, 600};
            result(double x, double y) = Griewank_f(x,y);
            break;
        case 1: 
            double range[2] = {-32, 32};
            result(double x, double y) = Ackley_f(x,y);
            break;
        case 2: 
    ...
    if (nDimensions == 0)
        nDimensions = 2;
    if (nParticles == 0)
        nParticles = 50; //how many particles do we use?
    if (mVelocity == 0)
        mVelocity = 60;
    if (nIterations == 0)
        nIterations = 1000;
