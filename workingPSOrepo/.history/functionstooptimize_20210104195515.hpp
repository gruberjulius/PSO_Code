...
//#define PI 3.141592
#define variable_datatype double



#define PI ((variable_datatype)acos(-1))
...
{
    variable_datatype  opcos = - cos(x) * cos(y),
            xpart = (x-PI)*(x-PI),
            ypart = (y-PI)*(y-PI),
            opexp = exp( -( xpart + ypart ) );
...
{
    variable_datatype  optrigo = sin(x) * cos(y),
            opexp   = exp( fabs( 1 - sqrt( x*x + y*y )/PI ) );

    return fabs( optrigo*opexp );
}

double ackley(variable_datatype x, variable_datatype y)
{
    variable_datatype  op1 = 20 * exp( -0.2 * sqrt( 0.5 * (x*x + y*y) ) ),
            op2 = exp( 0.5 * ( cos( 2*PI*x ) + cos( 2*PI*y ) ) );

    return -op1 - op2 + exp(1) + 20;
}
...
double VERY_Complicated(variable_datatype x, variable_datatype y){
    ...
    variable_datatype tmp2 = cos(x / 1) * cos(y / sqrt(2));
    variable_datatype temp = sin(20. - 20. * exp(-0.2 * sqrt(0.5 * tmp1)) - exp(0.5 * tmp2));
...
double Schwefel2_f(variable_datatype x, variable_datatype y){
    variable_datatype ret = x * sin(sqrt(fabs(x)) +  y * sin(sqrt(fabs(y))));
    return -ret;
}
...
double Ackley_f(variable_datatype x, variable_datatype y){
    ...
    return 20. - 20. * exp(-0.2 * sqrt(0.5 * tmp1)) - exp(0.5 * tmp2);
}
