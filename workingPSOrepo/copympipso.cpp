...
int main(int argc, char *argv[]) {
    ...
    long nIterations = strtol(argv[1], NULL, 10);
...
//printf("%d distributed_particles\n",distributed_particles );
    ...
    //MPI_Finalize();
        ...
        if(!basic_file){
            ...
            w_file_name += "_";
            w_file_name += std::to_string(nIterations); //make this C ready
            w_file_name += "_";
            w_file_name += std::to_string(nParticles); //make this C ready
            //w_file_name += "_m";
        }
        w_file_name += ".dat";

        std::ofstream times(w_file_name, std::ios::app); //make this C ready
        times << s_case << " " << size << " " << time_overhead << " " << gBestFitness << "\n";
        times.close();
    }
