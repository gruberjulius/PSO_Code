...
def Sorting(Index, Thread, Time, BestFitnessValue):
    ...
    print("keep your head up, we are currently working: ")
...
def fillupp(Index, Thread, Time, Bestfitnessvalue):
    ...
    numberofdifferentthreads = len(np.unique(Thread)) # 13 oder 14
    numberofcurrentdata = 0 #np.zero((numberofdifferentindicies, numberofdifferentthreads))
    newIndex = np.array(numberofdifferentindicies * numberofdifferentthreads * 50)
    newThread = np.array(numberofdifferentindicies * numberofdifferentthreads * 50 )
    newTime = np.array(numberofdifferentindicies * numberofdifferentthreads * 50)
    newBestFV = np.array(numberofdifferentindicies * numberofdifferentthreads * 50)
        ...
        while i < numberofdifferentindicies:
            while j < numberofdifferentthreads:
                ...
                if Index[k1] == i and Thread[k1] == j:
                    ...
                    newIndex[k1] = Index[old_k]
                    newTime[k1] = Time[old_k]
                    newThread[k1] = Thread[old_k]
                    newBestFV[k1] = Bestfitnessvalue[old_k]
                j += 1
            if numberofcurrentdata < 50:
                ...
                for m in range(0,l):
                    k2 += 1
                    newIndex[k2] = Index[old_k]
                    newTime[k2] = Time[old_k]
                    newThread[k2] = Thread[old_k]
                    newBestFV[k2] = Bestfitnessvalue[old_k]
            j = 0
            i += 1
        i = 0
        k2 += 1
