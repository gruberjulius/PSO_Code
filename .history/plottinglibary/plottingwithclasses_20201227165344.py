...
class dataclass(obejcts):
    ...
    def readin(storagelocation):
        self._Index, self._Thread, self._Time, self._BestFV = np.loadtxt(Dataset,delimiter=' ', unpack=True)
    ...
    def prepare(self._Index, self._Thread, self._Time, self._BestFV):
        ...
        ConvertDataDeviations(self._Time, PlottingDeviation, Samplesize)
        ConvertData(self._Index, PlottingIndex)
        ConvertData(self._Thread, PlottingThread)
        ConvertData(self._Time, PlottingTime)
        ConvertData(self._BestFV, PlottingBestFitnessValue)
