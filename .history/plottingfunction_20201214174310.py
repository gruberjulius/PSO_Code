...
def ConvertData(Format, PlottingFormat, NumberofSamples):
    ...
    """
    for i range(PlottingFormat):
        for j in range(PlottingFormat[0]):
            PlottingFormat[i][j] = hmean(Format[(i + j):(i + j + 1)])
...
def ConvertDataDeviations(Time, PlottingDeviation, NumberofSamples):
    ...
    """
    for i range(PlottingDeviation):
        for j in range(PlottingDeviation[0]):
...
def Prepare(Index, Thread, Time, BestFitnessValue):
    ...
    PlottingDeviation = ConvertDataDeviations(Time, PlottingDeviation, NumberofSamples)
...
def main():
    ...
    Prepare(Index, Thread, Time, BestFitnessValue)
