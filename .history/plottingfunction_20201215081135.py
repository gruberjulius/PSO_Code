...
def numberofsamplesforIndexandThreads(Index, Thread):
    ...
    """
    Differentindicies = countDistinct(Index)
...
def modifiedsortIndex(Index, Threads, Time, BestFitnessValue):
    ...
    I bugchecked the function seperatly
    """
    n = len(Index)
    for i in range(n):
...
def Sorting(Index, Thread, Time, BestFitnessValue):
    ...
    I checked the function seperatly and it works.
    """
...
def ConvertDataDeviations(Time, PlottingDeviation, NumberofSamples):
    ...
    a = shape[0]
    b = shape[-1]
    print("this is from the dta conversion function")
    print(a)
    print(b)
    while i < a:
        while j < b:
            PlottingDeviation[i][j] = np.std(Time[NumberofSamples*(i + j):
                NumberofSamples*(i + j + 1)])
            #the iteration is probably wrong
            j += 1
        i += 1
...
"""
Things I need to do:
Fix the input in the main function so that it does not so ugly
"""