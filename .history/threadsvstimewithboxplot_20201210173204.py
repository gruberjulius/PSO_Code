...
def cleaning(Threads, Time, stepsize, numoffunc, Deviations):
    size = len(Threads)

    # get the number of different threads sizes used
    modifiedTime = np.empty(numoffunc)
    ...
    while(i < numoffunc):
        modifiedTimes[i] = np.average(
            Time[(i * stepsize):((i * stepsize) + stepsize)])
        modifiedDeviations[i] = np.std(
            Time[(i * stepsize):((i * stepsize) + stepsize)])
        i += 1
    
    Threads = np.copy(modifiedThreads)
    Time = np.copy(modifiedTimes)
...
def main():
    ...
    Deviations = np.empty(numberofthreads)
    #Threads, mTimes, mDeviations = cleaning(Threads, Time,
    #                                         stepsize, numberofthreads, Deviations)
    cleaning(Threads, Time, stepsize, numberofthreads, Deviations)

    # plotting function
    plotJulius(Times, Threads, Deviations, nameoffunc,
