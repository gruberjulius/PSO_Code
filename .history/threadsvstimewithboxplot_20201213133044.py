...
def DistinctElements(arr):
    """Input: An Array 
    Ouput: The distinct elements of the array in a string
    """
    sol = np.array(arr[0])
    n = len(arr)
    # Pick all elements one by one 
...
def cleaningandplotting(Index, Threads, Time, BestFitnessvalue):
    ...
    for i in range(countDistinct(Index)): #loop over all numbers of indicies
        IndexforPlotting = np.array(Index[0])
        ThreadsforPlotting = np.array(Index[0])
        TimeforPlotting = np.array(Time[0])
        Bestfittnesforplotting = np.array(BestFitnessvalue[0])
            for j in range(len(Index)):
                if differentIndicies[i] == Index[j]:
                    np.append(ThreadsforPlotting, Threads[j])
                    np.append(TimeforPlotting, Time[j])
                    np.append(Bestfittnesforplotting, BestFitnessvalue[j]) 
