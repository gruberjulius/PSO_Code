    ...
    while k < len(Index):
        while i < numberofdifferentindicies:
            while j < numberofdifferentthreads:
                old_k = k
                if Index[k] == i and Thread[k] == j:
                    numberofcurrentdata +=1
                    newIndex[k] = Index[old_k]
                    newTime[k] = Time[old_k]
                    newThread[k] = Thread[old_k]
                    newBestFV[k] = Bestfitnessvalue[k]
                j += 1
            if numberofcurrentdata < 50:
                ...
                for m in range(0,l):
                    ...
                    newTime[k] = Time[old_k]
                    newThread[k] = Thread[old_k]
                    newBestFV[k] = Bestfitnessvalue[old_k]
            j = 0
            i += 1
        i = 0
        k +=1       
