...
def plot(Index, Thread, Time, Deviation, storagelocation ='', 
    ...
    i = 0
    numberofdifferentindicies = np.size(Index, axis = 1)
    print("checking if slicing works")
    print(Deviation[i:(i+ 1)])
    while i < numberofdifferentindicies:
        DeviationDummy = Deviation[i:(i + 1)].flatten()
        plt.errorbar(Thread[i:(i+ 1)], Time[i:(i + 1)], DeviationDummy)
    
    plt.savefig( storagelocation + nameoftheplot + 'chef'+'.pdf' )
...
def main():
    ...
    plot(PlottingIndex, PlottingThread, PlottingTime, PlottingDeviation, '', 'bro')
