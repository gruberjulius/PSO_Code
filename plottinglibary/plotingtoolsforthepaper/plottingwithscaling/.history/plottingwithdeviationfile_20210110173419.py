    ...
    while j < numberofdatasets:
            ...
            if constant =="y":
                ...
                while i < k :#numberofdifferentindicies
                    ...
                    Time2Dummy = TimeDummy[0]
                    Thread2Dummy = ThreadDummy[0]
                    plt.errorbar(Thread2Dummy, Time2Dummy, Deviation2Dummy, fmt = "o",label = labelname)#, elinewidth=1, capsize=3, capthick=3, color = coloursplot[j])
                    #code for riksha : plt.errorbar(Thread2Dummy, Time2Dummy, Deviation2Dummy, fmt = "o",label = labelname, elinewidth=1, capsize=3, capthick=3, color = coloursplot[j])
                    #, uplims=upperlimits, lolims = lowerlimits)#, barsabove = 'True')#,, fmt='d' uplims=True, lolims=True)#, linestyle='None', fmt='o',  label = labelname)
                    i += 1
            j += 1
