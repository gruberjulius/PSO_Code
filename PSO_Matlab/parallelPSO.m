...
parfor (ipp_j=1:problem_data.problem_dimensions,) % sets range
...
parfor (j=1:problem_data.problem_dimensions,) %creates velocity range
    velocity_range(j) = problem_data.VelocityFraction * (problem_data.position_upperbound(j)-problem_data.position_lowerbound(j));

end

parfor (i=1:problem_data.NumParticles,) %randomly creates initial velocities
...
parfor (i=1:problem_data.NumParticles,) %sets all particle inertia values to the InitialInertia
...
parfor (i=1:problem_data.NumParticles,) %solve for function values
...
parfor i=1:problem_data.NumParticles, %set each initialized value to best value
    for j=1:problem_data.problem_dimensions,
...
parfor (i=1:problem_data.NumParticles),
